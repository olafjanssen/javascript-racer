    ...
    this.update = function () {
        ...
        if (tick % interval === 0) {
            ...
            if (!ai){
                agent.epsilon = 1;
                agent.alpha = 0.05;
                var action = agent.act(environment.getState(), environment.readAction()); console.log(environment.readAction(), action);
            } else {
                agent.epsilon = 0.1;
                agent.alpha = 0.05;
                var action = agent.act(environment.getState()); console.log('re-action', action);
...
var Environment = function (game) {
    ...
    function getReward() {
        ...
        var acc = game.speed / game.maxSpeed - oldSpeed;

        oldSpeed = game.speed / game.maxSpeed;
        return acc * 100 + oldSpeed;
    }
